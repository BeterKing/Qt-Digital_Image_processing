...
void CaptureScreen::initWindow()
{
    ...
    if (cur_screen) {
        scaledRate = cur_screen->devicePixelRatio();
    }
}
...
{
    m_capturePixmap = m_loadPixmap.copy(copyImg);
...
QRect CaptureScreen::getRect(const QPoint &beginPoint, const QPoint &endPoint)
{
    ...
    copyImg = QRect(x*scaledRate,y*scaledRate,width*scaledRate,height*scaledRate);
